...
forcesync() {
  ...
  blockhash=`curl -s https://explorer.polispay.org/api/getblockhash?height=$blockcount`
  echo "| ଓ hash : $blockhash"
  curl -s https://explorer.polispay.org/block/$blockhash > /dev/null
  echo "╘═══════════════════════════════>>"
}
...
main() {
  ...
  currentblockcount=`curl -s https://explorer.polispay.org/api/getblockcount`
